...
public class SetService {
	...
	/* Set(집합, 주머니)
	 * - 기본적으로 순서를 유지하지 않음
	 *   (index 없음! -> get() 메더스가 없음)
	 *   
	 * - 중복 데이터 저장 X (같은 객체가 들어오면 덮어쓰기)
	 * 
	 * Set 인터페이스의 자식 클래스
	 * 
	 * 1. HashSet (대표) : 처리속도가 빠른 Set ( 가장 많이 씀 )
	 * 
...
//			     -> 화살표(커서)를 한칸 움직임
	 ...
	 *  
	 *  1. Object.eqauls() 오버라이딩 하기		==> 동등비교
	 *  2. Object.hashCode() 오버라이딩 하기	==> 동일비교
	 *  
	 *  ** 1,2 둘다 수행해야만 함
...
//		set.add(new Person("이름", 1024, '화'));
//		set.add(new Person("유리", 5, '수'));
//		set.add(new Person("맹구", 5, '목'));
//		set.add(new Person("짱구", 5, '남'));
//		set.add(new Person("짱구", 5, '남'));
//		set.add(new Person("짱구", 5, '남'));
//		set.add(new Person("짱구", 5, '남'));
//		set.add(new Person("짱구", 5, '남'));
...
//		System.out.println("길이 : " + set.size());
	...
	public void test4() {
		...
		while (lotto.size() < 6) {	// => 중복된 객체는 지워주니까 
									// 안곂치는것 6개 나올때까지 계속 실행
	...
	public void lottoNumberGenerator() {
		...
		for (int i = 0; i < price/1000 ; i++) {
			...
			Set<Integer> lottoRow = new TreeSet<Integer>();
			
			// 1회치에 해당되는 로또번호 생성
			while(lottoRow.size() < 6) {
				lottoRow.add( random.nextInt(45)+1 );
			}
			// lottoList에 추가
			lottoList.add(lottoRow);
		}
